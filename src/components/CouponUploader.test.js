...
function makeJsonFile(content, name = 'coupons.json') {
...
describe('CouponUploader', () => {
    ...
    describe('Initial render', () => {
        it('renders the page heading', () => {
            render(_jsx(CouponUploader, {}));
            expect(screen.getByText('Coupon Uploader')).toBeInTheDocument();
        });
        it('renders the drop zone with instructions', () => {
            render(_jsx(CouponUploader, {}));
            expect(screen.getByText(/drag and drop the coupons file/i)).toBeInTheDocument();
        });
        ...
        it('renders the Upload button', () => {
            render(_jsx(CouponUploader, {}));
            expect(screen.getByRole('button', { name: /upload coupons/i })).toBeInTheDocument();
        });
    ...
    describe('handleFileChange – file input', () => {
        ...
        it('shows toast.error and throws for a non-JSON file', () => {
            ...
            const input = document.querySelector('input[type="file"]');
            expect(() => fireEvent.change(input, { target: { files: [txtFile] } })).toThrow('Invalid file type');
        ...
        it('shows toast.error when the JSON content is not an array', () => {
            ...
            expect(toast.error).toHaveBeenCalledWith('Invalid format, expected an array of coupons');
        });
        it('hides the empty-state message after coupons are loaded', () => {
            ...
            expect(screen.queryByText(/no coupons to loaded/i)).not.toBeInTheDocument();
        });
    });
    // ── Drag and drop ──────────────────────────────────────────────────────────
    describe('Drag and drop', () => {
        ...
        it('shows toast.error when a non-JSON file is dropped', () => {
            ...
            expect(() => fireEvent.drop(dropZone, { dataTransfer: { files: [txtFile] } })).toThrow('Invalid file type');
            expect(toast.error).toHaveBeenCalledWith('Invalid file type');
        });
    });
    // ── Upload button ──────────────────────────────────────────────────────────
    describe('handleUpload', () => {
        ...
        it('calls axios.post and shows success toast on a successful upload', async () => {
            ...
            await userEvent.click(btn);
            await waitFor(() => {
                expect(mockedAxios.post).toHaveBeenCalledWith('https://jsonplaceholder.typicode.com/posts', VALID_COUPONS);
                expect(toast.success).toHaveBeenCalledWith('Coupons Uploaded to the Server Successfully');
            });
        });
